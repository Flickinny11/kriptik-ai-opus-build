    ...
    private async saveApp(app: ImportedApp): Promise<void> {
        ...
        try {
            await db.insert(externalApps).values({
                // Note: id is auto-generated by the schema's defaultFn
                userId: app.userId,
                sourceRepo: app.sourceRepo,
                branch: app.branch,
                framework: app.framework as 'nodejs' | 'python' | 'react' | 'nextjs' | 'express' | 'fastapi' | 'flask' | 'django' | 'other',
                structure: app.structure as any,
                integrationPoints: app.integrationPoints as any,
                envRequirements: app.envRequirements,
            });
        } catch (error) {
