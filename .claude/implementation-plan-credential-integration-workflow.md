...
1. Analyze NLP prompt for integration keywords:
   ...
   Analyze NLP's for nontechnical user phrases that will not indicate a specific platform for integration to determine what the best, most current, newest to the day, integrations and platforms to perform the user's requested functions, since users typically will request a function rather than a platform for integration - a user would request features and capabilities that need to be parsed to understand what the user actually wants, and then to determine what would be needed to create that function, and what would be needed to create an app with that function, and how to tie it all together. there will be both technical and nontechnical users.
   *Stack detection will be completely dynamic, based on determining the user's intent from the functions, features, capabilities that they want to create. This is the true 'intent' lock miracle of kriptik; being able to understand the user's prompt and break it downn into what would actually satisfy the user for functionality, and then what integrations would be required to create that functionality, and ultimately, what else does the user want that they're not telling kriptik. does the user want their app to make money? does the user want their app to be a personal app, paid app, how does the user want it to look, does it need design dependenciees like three.js, or does it need cloud compute like runpod, or does it need open source models and workflows, or what woud it take for the user to say "yep, that's what i wanted" when the app is complete.
