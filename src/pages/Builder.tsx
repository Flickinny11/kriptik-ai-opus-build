...
import { AgentDemoOverlay, type NarrationPlaybackSegment } from '../components/builder/AgentDemoOverlay';
...
export default function Builder() {
    ...
    // Get current project from store for Fix My App demo detection
    const { currentProject, updateProject } = useProjectStore();
    
    // Agent Demo state for completed Fix My App projects
    ...
    // Fix My App: Trigger agent demo on first view of completed project
    useEffect(() => {
        // Check if this is a completed Fix My App project that hasn't shown demo yet
        const isFixMyAppCompleted = currentProject?.fixingStatus === 'completed';
        const hasNotViewedDemo = !(currentProject as any)?.demoViewed;
        
        if (isFixMyAppCompleted && hasNotViewedDemo && projectId) {
            console.log('[Builder] Fix My App completed! Triggering agent demo...');
            
            // Generate demo segments based on project features
            // In production, this would call the backend to generate narration
            const introSegment: NarrationPlaybackSegment = {
                segment: {
                    id: 'intro',
                    text: `Welcome! Your app has been successfully rebuilt. Let me show you what KripTik AI has created based on your original intent.`,
                    action: 'wait',
                    duration: 3000,
                },
                audioBase64: '', // Would be generated by TTS in production
            };
            
            const featuresSegment: NarrationPlaybackSegment = {
                segment: {
                    id: 'features',
                    text: `I've implemented all the core features you requested. The app is now production-ready with proper error handling, authentication, and responsive design.`,
                    action: 'highlight',
                    duration: 4000,
                },
                audioBase64: '',
            ...
            const completionSegment: NarrationPlaybackSegment = {
                segment: {
                    id: 'complete',
                    text: `You can now take control and make any additional customizations. Click "Take Control" whenever you're ready!`,
                    action: 'wait',
                    duration: 3000,
                },
                audioBase64: '',
            ...
            setDemoSegments([introSegment, featuresSegment, completionSegment]);
            setShowAgentDemo(true);
            setActiveTab('preview'); // Switch to preview to show the demo
        }
    }, [currentProject?.fixingStatus, currentProject, projectId]);

    // Handle agent demo completion
    const handleAgentDemoComplete = () => {
        setShowAgentDemo(false);
        setDemoSegments([]);
        
        // Mark as viewed so we don't show again
        if (projectId && currentProject) {
            updateProject(projectId, { demoViewed: true } as any);
        }
    };

    const handleAgentDemoTakeControl = () => {
        console.log('[Builder] User took control of demo');
        handleAgentDemoComplete();
    };
    ...
    return (
        <SandpackProvider initialFiles={INITIAL_FILES}>
            ...
            >
                ...
                <AgentDemoOverlay
                    isActive={showAgentDemo}
                    segments={demoSegments}
                    onTakeControl={handleAgentDemoTakeControl}
                    onComplete={handleAgentDemoComplete}
                    showTranscript={true}
                />
...
// Quick action panels available via FloatingDevToolbar and Open Source Studio
